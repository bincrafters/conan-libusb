...
class LibUSBConan(ConanFile):
    ...
    def source(self):
        ...
        url = "https://github.com/libusb/libusb/releases/download/v%s/%s" % (self.version, package_name)
    ...
    def build(self):
        ...
        with environment_append(env_build.vars):
            ...
            self.run("cd %s && ./configure %s %s" % (self.release_name, prefix, library_type))
            self.run("cd %s && make" % self.release_name)
            self.run("cd %s && make install" % self.release_name)

    def package(self):
        # Copying headers
        self.copy(pattern="*.h", dst="include", src="%s/include" % self.prefix_install, keep_path=True)
        self.copy(pattern="*.la", dst="lib", src="%s/lib" % self.prefix_install, keep_path=False)
        self.copy(pattern="*.so*", dst="lib", src="%s/lib" % self.prefix_install, keep_path=False)
        self.copy(pattern="*.pc", dst="lib", src="%s/lib" % self.prefix_install, keep_path=True)
